...
describe("WorkerClient + WorkerHost (in-process)", () => {
  it("round-trips a small envelope through the seal/unseal protocol", async () => {
  ...
  it("emits KDF and seal progress before the sealed event", async () => {
    const { client, cleanup } = pair();
    try {
        ...
        client.seal(
          {
            canonicalSecret: makeSecret(),
            entries: [
              { path: "tiny.txt", bytes: new Uint8Array([1]), mode: 0o100644 },
            ],
            argon2: Argon2Params.MIN.toValue(),
      ...
      const phases = events
        .filter(
          (e): e is Extract<WorkerSealEvent, { kind: "progress" }> =>
            e.kind === "progress",
        )
  ...
  it("throws a WorkerClientError on authentication failure during unseal", async () => {
    const { client, cleanup } = pair();
    try {
      const salt = new Uint8Array(16);
      crypto.getRandomValues(salt);
        ...
        client.seal(
          {
            ...
            entries: [
              {
                path: "secret.txt",
                bytes: new TextEncoder().encode("sensitive data"),
                mode: 0o100644,
              },
            ],
            argon2: Argon2Params.MIN.toValue(),
  ...
  it("transfers buffers as zero-copy where the bus permits", async () => {
        ...
        client.seal(
          {
            canonicalSecret: makeSecret(),
            entries: [
              { path: "x.bin", bytes: new Uint8Array([0xff]), mode: 0o100644 },
            ],
            argon2: Argon2Params.MIN.toValue(),
            salt: new Uint8Array(16),
            noncePrefix: new Uint8Array(4),
      ...
      const iterable = client.seal(
        {
          canonicalSecret: makeSecret(),
          entries: [
            { path: "f.txt", bytes: new Uint8Array(64 * 1024), mode: 0o100644 },
          ],
